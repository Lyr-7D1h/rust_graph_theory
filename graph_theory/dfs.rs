...
/// Depth first search returns amount unvisited nodes
fn dfs_unvisited_nodes(alist: Vec<Vec<usize>>) -> usize {
  let mut visited_nodes = vec![false; alist.len()];
...
fn main() {
  ...
  println!("{}", dfs_unvisited_nodes(alist));
}
